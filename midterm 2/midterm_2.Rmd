...
```{r}
surgery %>% 
  select(ahrq_ccs, mortality_rsi) %>% 
  top_n(5, mortality_rsi) %>% 
  arrange(desc(mortality_rsi))
```

```{r}
surgery %>% 
  select(ahrq_ccs, complication_rsi) %>% 
  top_n(5, complication_rsi) %>% 
  arrange(desc(complication_rsi))
```
