...
(1) each variable has its own column?  
 -yes, variable are the drugs, not true that every variable(drug) has its own column
(2) each observation has its own row?  
-yes, observations are the patients 
(3) each value has its own cell?  
  -yes, heart rate 
...
```{r}
heartrate %>% 
  pivot_longer(-patient, #pt will not pivot
             names_to ="drug", #make new column
             values_to = "heartrate" #where values will be moved
             )
```
...
it is untidy because the individual range of income should not have its own column, the range of income should all be under one column
...
The days should be under one column
