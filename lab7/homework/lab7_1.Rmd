...
```{r}
penguins %>% 
  tabyl(island, species) #species are observations in this way, not tidy data 
```
...
```{r}
penguins %>%
  summarize(across(c(species, island, sex), n_distinct)) #c calls on the vector
```
...
penguins %>% 
  group_by(species) %>% 
  summarise(across(contains("mm"), sd, na.rm=T), total=n())
```

```{r}
penguins %>% 
  group_by(species) %>% 
  summarise(across(contains("mm"), mean, na.rm=T), total=n())
```
