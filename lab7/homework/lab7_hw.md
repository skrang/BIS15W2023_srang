...
```r
summary(amniota) # NAs are represented as -999
```
...
```r
glimpse(amphibio) #NAs are represented by NA
```
...
amniota_tidy %>% 
  group_by(class) %>% 
  summarize(sum(is.na(amniota_tidy$egg_mass_g)), total=n()) #mammals do not lay eggs hence why they are NAs as for birds and reptiles, the data may not have been collected thats why theres NAs
```

```
## # A tibble: 3 × 3
##   class    `sum(is.na(amniota_tidy$egg_mass_g))` total
##   <chr>                                    <int> <int>
## 1 Aves                                     15907  9802
## 2 Mammalia                                 15907  4953
## 3 Reptilia                                 15907  6567
```
...
```r
amphibio_lifestyle %>% 
  summarise_all(~(sum(is.na(.)))) #NAs could represent that the particular species only has one habitat style so the rest would be filled as NA
```
